...
class UsuariosController extends Controller
{
    ...
    {
    	$usuarios = Usuario::join('tiendas', 'usuarios.tienda', '=', 'tiendas.id')
            ->select('usuarios.*', 'tiendas.tienda')
            ->orderBy('id','desc')
            ->paginate(10);
            //dd($usuarios);
    ...
    public function registrar(UsuarioRequest $peticion)
    {
        ...
        //dd(Auth::user()->tienda); 
        $groups = ['groups' =>Tienda::orderBy('tienda','desc')->lists('tienda','id')];
        return view('usuarios.registrar',$groups,['verClave' => true]);
    }
    ...
    public function modificar(UsuarioRequest $peticion, $id)
    {
        ...
        return view('usuarios.modificar',$groups,['usuario' => $usuario, 'verClave' => false]);
    }
